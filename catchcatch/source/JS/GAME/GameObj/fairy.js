    ...
    normalAttack(magic) {
        ...
        switch (this.fairyNum) {
            ...
            case 3:
                ...
                if (this.isTriple) {
                    let magic2 = new Magic(thisScene, this);
                    let magic3 = new Magic(thisScene, this);
                    magics.add(magic2);
                    magics.add(magic3);
                    magic2.anims.play("magic" + this.fairyNum, true);
                    magic3.anims.play("magic" + this.fairyNum, true);
                    let num = (this.x - (input.x + camera.scrollX)) ** 2 + (this.y - (input.y + camera.scrollY)) ** 2;
                    let d = 145;
                    let angle_dis = Math.sqrt(num);
                    let angle_mouse = Math.asin(-((input.y + camera.scrollY) - this.y) / angle_dis);
