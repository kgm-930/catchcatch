export default class Magic extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(scene, fairy) {
    ...
    if (fairy.fairyNum === 2) {
      scene.time.addEvent({ delay: 300, callback: () => { this.body.checkCollision.none = false; this.setVisible(true); if (this.texture.key === "magic2") { this.setVelocity(0, 0); } }, loop: false });
    }
    if (fairy.fairyNum === 5) {
      scene.time.addEvent({ delay: 1000 * fairy.bombtime, callback: () => { this.bomb(); }, loop: false });
      bombs.add(this);
    } else {
  ...
  bomb() {
    ...
    thisScene.time.addEvent({ delay: (this.fairy.range * 1000), callback: () => { console.log(this); this.destroy(); }, loop: false });
  }
